...
async function calculateMonthlyAnalytics() {
    try {
        ...
        // Get all datasets from Dataset Service via HTTP with retry
        let allDatasets = [];
        const DATASET_SERVICE_URL = process.env.DATASET_SERVICE_URL || 'http://dataset-service:7002';
        let retries = 0;
        const maxRetries = 3;
        
        while (retries < maxRetries && allDatasets.length === 0) {
            try {
                console.log(`[Analytics Monthly] Fetching datasets from ${DATASET_SERVICE_URL}/api/datasets?perPage=1000 (attempt ${retries + 1}/${maxRetries})`);
                const response = await axios.get(`${DATASET_SERVICE_URL}/api/datasets?perPage=1000`, {
                    timeout: 5000
                });
                
                if (response.data && response.data.items) {
                    allDatasets = response.data.items;
                    console.log(`[Analytics Monthly] Got ${allDatasets.length} datasets from Dataset Service`);
                    break;
                }
            } catch (error) {
                retries++;
                console.error(`[Analytics Monthly] Error fetching datasets (attempt ${retries}/${maxRetries}):`, error.message);
                
                if (retries < maxRetries) {
                    console.log(`[Analytics Monthly] Retrying in 2 seconds...`);
        ...
        if (allDatasets.length === 0) {
            console.error('[Analytics Monthly] Make sure dataset-service is running on', DATASET_SERVICE_URL);
        }
        ...
        allDatasets.forEach(dataset => {
            ...
            const monthString = `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, '0')}`;
